...
TERMINATION_PASS = input(green("Please enter your termination password (case sensitive){DEFAULT: 12345}: "))
if TERMINATION_PASS == "": print(red("KEPT UNCONFIGURED!!  ") + green("Default: 12345"))
MODES_SOUND = ["1","2",""]
MODE = input(green("[1]-Protection Mode {DEFAULT}, [2]-Monitor Mode: "))
while MODE not in MODES_SOUND:
    MODE = input(green("[1]-Protection Mode {DEFAULT}, [2]-Monitor Mode: "))
if MODE == "":
    MODE = "1"
    print(red("KEPT UNCONFIGURED!!  ") + green("Default: [1]-Protection Mode"))
SOUND = input(green("Sound Notification when started: [1]-ON {DEFAULT}, [2]-OFF: "))
while SOUND not in MODES_SOUND:
    SOUND = input(green("Sound Notification when started: [1]-ON {DEFAULT}, [2]-OFF: "))
if SOUND == "":
    SOUND = "1"
    print(red("KEPT UNCONFIGURED!!  ") + green("Default: Sound Notification when started: [1]-ON"))
...
if SOUND == "1": RAWUA[1] += "Sound Notification when started: ON}"
if SOUND == "2": RAWUA[1] += "Sound Notification when Started: OFF}"
RAWUA[1] += "\n"
