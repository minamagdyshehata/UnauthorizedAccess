...
def Register():
    ...
    winreg.SetValueEx(keyCURRENTRUN, scriptname, 0, winreg.REG_SZ, chr(34) + scriptfullpath + chr(34))
...
def SendNotification():
    ...
    Log = open(sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + machinename +"Log.txt", "a")
    Log.write("Unauthorized Access on " + TimeStamp + chr(10))
    Log.close()
    if os.path.isfile(sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + "CommandCam.exe"):
        try:
            subprocess.call(chr(34) + sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + "CommandCam.exe" + chr(34) + " /filename " + chr(34) + sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + TimeStampFormatted + ".jpg" + chr(34),shell=True)
        except:
            pass
    online = is_connected()
    if online:
        try:
            if not os.path.isfile(sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + "CommandCam.exe"):
                subprocess.call("powershell (new-object System.Net.WebClient).DownloadFile('https://raw.githubusercontent.com/minamagdyshehata/UnauthorizedAccess/master/CommandCam.exe','" + sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + "CommandCam.exe')",shell=True)
                subprocess.call(chr(34) + sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + "CommandCam.exe" + chr(34) + " /filename " + chr(34) + sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + TimeStampFormatted + ".jpg" + chr(34),shell=True)
        except:
            pass
        try:
            ...
            if os.path.isfile(sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + TimeStampFormatted + ".jpg"):
                attach = sys.argv[0][:-len(os.path.basename(sys.argv[0]))] + TimeStampFormatted + ".jpg"
            ...
            pass
    subprocess.call("shutdown /S /F /T 01",shell=True)
    Terminate()
